...
\begin{frame}
  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
      \begin{block}{Ilustrasi letak \textit{node} 4}
      \begin{figure}
  ...
  \begin{itemize}
    ...
    \item Model termal dapat memprediksi lebih dari 95\% tren atau variasi perubahan suhu pada 5 \textit{node} dan memiliki deviasi standar kesalahan prediksi suhu di bawah 1 $^\circ$C untuk 6 \textit{node}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Metode \textit{machine learning} dapat digunakan untuk membuat model termal semi-empiris satelit
    \item Model termal semi-empiris satelit tersebut berhasil diimplementasikan pada satelit LAPAN-A3
    \item Untuk kedua periode observasi satelit LAPAN-A3, prediksi 5 dari 7 \textit{node} satelit menghasilkan skor $R^2$ lebih besar daripada 0.95 dan 6 dari 7 \textit{node} satelit menghasilkan nilai RMSE di bawah 1 $^\circ$C
  \end{itemize}
\end{frame}
