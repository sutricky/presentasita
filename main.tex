...
\begin{frame}
  ...
  \begin{columns}[T]
    ...
    \column{0.6\textwidth}
      \begin{itemize}
        \item Memiliki sistem kendali termal pasif berupa struktur alumunium yang diberi anode hitam
...
\begin{frame}
  ...
  \begin{columns}[T]
    \column{0.39\textwidth}
    \begin{itemize}
      \item Pemodelan termal konvensional mengaruskan perhitungan semua variabel pada ruas kanan persamaan laju perubahan suhu \textit{node} (Persamaan (\ref{eq:lineq}))
      \item Dengan metode regresi linear \textit{machine learning}, variabel yang masih harus dicari hanya variabel yang berubah terhadap waktu (diberi tanda (t) pada Persamaan (\ref{eq:lineq})) atau hasil perkaliannya
    \end{itemize}
...
\begin{frame}
  ...
  \begin{columns}[c]
    ...
    \begin{itemize}
      ...
      \item Metode \textit{machine learning} umum digunakan pada permasalahan yang melibatkan data dalam jumlah banyak seperti pengelompokkan wajah manusia, pembuatan rekomendasi produk, analisis finansial modern
    \end{itemize}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{block}{\center 1. Persiapan dataset dasar}
      ...
      \begin{itemize}
        ...
        \item Data waktu pembacaan sensor-sensor satelit dipisah menjadi
format tahun, bulan, tanggal, jam, menit, dan detik
      \end{itemize}
    \end{block}
    \column{0.02\textwidth}
    \column{0.49\textwidth}
  ...
  \begin{columns}[T]
    \column{0.49\textwidth}
      \begin{block}{\center 3. Penyaringan dataset}
        Penyaringan dilakukan untuk menghilangkan kesalahan dan \textit{outlier} bacaan sensor karena dapat mengurangi akurasi model termal.
...
\begin{frame}
  ...
  \begin{columns}[c]
    ...
    \begin{block}{\center \normalsize Kapasitas termal rata-rata \textit{node}}
      ...
      \item Rentang kapasitas termal besar dapat menyebabkan perbedaan perubahan suhu antar komponen
      ...
      \item Jumlah \textit{node} dapat ditambah sehingga karakteristik termal satelit dapat diwakili lebih menyeluruh
      \end{itemize}
    \end{block}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{block}{\center Pengaruh perubahan suhu \textit{node} lain}
      \begin{itemize}
        ...
        \item Diperlukan analisis lebih lanjut untuk menentukan variabel dominan yang mungkin menghasilkan \textit{error} besar pada Persamaan (\ref{eq:lineq})
    ...
    \begin{block}{\center Asumsi bentuk satelit}
      \begin{itemize}
        \item Satelit diasumsikan berbentuk balok sehingga \textit{node-node} dianggap berbentuk plat persegi panjang
        \item Ada komponen satelit yang tidak berbentuk persegi panjang seperti antena dan kamera
        \item Nilai \textit{view factor node} dapat dihitung terpisah menggunakan metode numerik lain
      \end{itemize}
    \end{block}
  \end{columns}
\end{frame}
